...
app.post('/api/signup', async (req, res) => {
  ...
  try {
    ...
    // 2. Add them to your new visual user_roles table (defaults to false)
    const { error: dbError } = await supabase
      .from('user_roles')
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const isAdmin = roleData ? roleData.is_admin : false;
...
app.patch('/api/users/:id/role', async (req, res) => {
  const { id } = req.params;
  const { role } = req.body; //admin or user blabla
  ...
  try {
    ...
    if (error) throw error;
    res.json({ success: true, message: `User upgraded to ${role}` });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
